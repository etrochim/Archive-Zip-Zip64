...
use constant ZIP64ARCHIVECLASS => 'Archive::Zip::Zip64::Archive';
...
sub readFromFileHandle {
    ...
    # This should be 0 unless this is a zip64 archive, where it will be
    # handled later 
    ...
    if($zip64EOCDLocatorSignature == ZIP64_END_OF_CENTRAL_DIRECTORY_LOCATOR_SIGNATURE) {
        ...
        # This should be 0
        $self->{'eocdOffset'} = $eocdPosition - $self->centralDirectorySize
          - ( $self->zip64SizeOfEndOfCentralDirectory + 12 )
    ...
    for ( ; ; ) {
        ...
        return $status if $status != AZ_OK;
        last           if $signature == END_OF_CENTRAL_DIRECTORY_SIGNATURE || $signature == ZIP64_END_OF_CENTRAL_DIRECTORY_SIGNATURE;
